...
> **当前实现状态：** 真实采集、Inngest 长任务、AI 探针、浏览器级渲染与 GSC read-only OAuth 均已进入应用。GSC 采用平台托管 OAuth：运营方配置一次 OAuth Client 与平台回调地址，用户只在具体项目详情页授权 Google 账号并选择已授权的 property。
...
| 平均自然排名 | 平台托管 GSC OAuth；在项目详情页连接 Google 账号并选择已授权 property |
...
- `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` —— 平台托管 GSC OAuth Client
- `GOOGLE_OAUTH_REDIRECT_URI` —— 当前 Veris 部署域名的 `/api/gsc/callback`，须与 Google Cloud 中登记的地址逐字符一致

生产环境用真实 Turso URL + token；本地用 `file:./veris.db` + 占位 token（见上）。

页面渲染对比依赖**托管浏览器 API**（Vercel 无法自带 chromium），长时任务由 **Inngest** 承担（Vercel 友好，避免进程内后台任务）。
