...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // 只有 collecting 的 run 才订阅 realtime channel。其余状态（collected/failed、demo/seed
  // 的 reviewing、或采集早已结束后重连）不会再有消息发到 channel——若仍订阅，for await 永不
  // 收到终止帧、流永不关闭、连接挂到平台超时。这里直接一次性回终止帧并关闭。
  if (run.status !== 'collecting') {
    const msg: RunProgressMessage = run.status === 'failed' ? { type: 'failed', reason: 'run_failed' } : { type: 'done' }
