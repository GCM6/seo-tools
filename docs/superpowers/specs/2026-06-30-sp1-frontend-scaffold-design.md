...
| 样式 | Tailwind v4 + CSS 变量主题 | 映射原型的证据等级 token |
...
CSS 变量映射原型 token：
- 证据等级配色：`--measured`(实测/青 #0B6E74) `--inferred`(推断/橙 #B26B16) `--gap`(差距/红 #B23A48) `--good`(已具备/绿 #2E7D56)，含各自浅底色。
- 字体：Space Grotesk(display) / Inter(body) / JetBrains Mono(mono) / Noto Sans SC。
- 中性色、圆角、线色等照原型 `:root`。
