...
4. [ ] 开启访问控制：最小解 Vercel Deployment Protection（Settings → Deployment Protection，Standard Protection 即可挡全网匿名访问；将来做多用户体系再替换为应用内鉴权）。注意：开启后 `/share` 分享链接也会被挡，若 G3 需要对外分享报告，改用 Protection Bypass 或提前实现应用层鉴权——此取舍需在 G3 招募前决策。
...
GSC 是唯一的一手搜索表现数据，采用 Veris **平台托管**的 read-only OAuth。OAuth Client、Client Secret 与回调地址由平台运营方配置一次；普通用户不填写凭据，只在项目详情页授权 Google 账号并选择站点。redirect 必须是平台生产 URL，而不是被诊断网站 URL。
...
2. [ ] Authorized redirect URI 精确填：
   ...
   ```
3. [ ] Vercel 补平台三件套 → redeploy：
   - `GOOGLE_OAUTH_CLIENT_ID`
   - `GOOGLE_OAUTH_CLIENT_SECRET`
   - `GOOGLE_OAUTH_REDIRECT_URI=https://<生产域名>/api/gsc/callback`（须与控制台完全一致）
4. [ ] 验证：普通用户在项目详情页点「连接 GSC」→ 走完 Google 授权 → 跳回项目页显示已连接、站点下拉能列出 property（`/api/gsc/sites`，SP-A1 接的）。
...
- [ ] `curl -s https://<生产域名>/robots.txt` 应输出 `Disallow: /` 与 `Allow: /share`。
- [ ] `curl -s https://<生产域名>/zh/projects | grep -i noindex` 应命中 meta robots。
...
| `GOOGLE_OAUTH_CLIENT_ID` / `_SECRET` / `_REDIRECT_URI` | 建议 | 平台托管 GSC OAuth（redirect=平台生产 `/api/gsc/callback`，不向普通用户暴露） |
